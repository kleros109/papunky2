...
// Define an interface for the Spotify oEmbed response to fix TypeScript errors
...
const customTools = [
  ...
  },
  {
    ...
    function: {
        name: 'get_track_info',
        description: 'Get track metadata, including album art, from a Spotify track URL using the oEmbed API.',
        parameters: {
            type: 'object',
            properties: {
                spotifyUrl: { type: 'string', description: 'The full Spotify track URL.' }
            },
            required: ['spotifyUrl']
        }
    }
...
export async function executeTool(name: string, args: Record<string, unknown>): Promise<ToolResult> {
  try {
    switch (name) {
      ...
      case 'get_track_info': {
        const spotifyUrl = args.spotifyUrl as string;
        if (!spotifyUrl || typeof spotifyUrl !== 'string') {
            return { error: 'A valid Spotify URL is required.' };
        }
        try {
            const oembedUrl = `https://open.spotify.com/oembed?url=${encodeURIComponent(spotifyUrl)}`;
            const response = await fetch(oembedUrl);
            if (!response.ok) {
                throw new Error(`Spotify oEmbed API returned status ${response.status}`);
            }
            const data = await response.json() as SpotifyOembedResponse;
            // The API returns thumbnail_url, title, etc.
            // Title format is often "Song Name - Artist Name"
            const titleParts = (data.title || '').split(' - ');
            const songTitle = titleParts[0];
            const artistName = titleParts.slice(1).join(' - ');
            return {
                thumbnail_url: data.thumbnail_url,
                albumTitle: data.title, // oEmbed gives a generic title, we'll use it as album title for now
                songTitle: songTitle,
                artistName: artistName,
            };
        } catch (e) {
            const error = e instanceof Error ? e.message : 'Unknown error fetching from Spotify oEmbed API';
            return { error };
        }
      }
      default: {
